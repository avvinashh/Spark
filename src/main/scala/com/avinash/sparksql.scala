...
object sparksql {
  def main(args: Array[String]): Unit = {
   // val conf=new SparkConf().setAppName("Spark sql example").setMaster("local")
    ...
    val df1=spark.read.format("csv").option("header", "true").load("/FileStore/tables/Telecomdata.csv")
    val df2=spark.read.format("csv").option("header", "true").load("/FileStore/tables/Telecomdatacustomer.csv")
